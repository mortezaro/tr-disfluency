...
import spacy
tokenizer = spacy.load("en_core_web_sm", disable=["tagger", "parser", "disf"])
...
class disf_Dataset(data.Dataset):
...
class Bert_CRF(BertPreTrainedModel):
    ...
    def forward(self, input_ids, attn_masks, labels=None):  # dont confuse this with _forward_alg above.
        ...
        if labels is not None:
            ...
            loss = -self.crf(log_soft(emission, 2), labels, mask=attn_masks)
...
def generate_training_data(config, bert_tokenizer="bert-base", do_lower_case=True):
    ...
    train_dataset = disf_Dataset(tag2idx, train_sentences, train_labels, tokenizer_path = bert_tokenizer, do_lower_case=do_lower_case)
    ...
    dev_dataset = disf_Dataset(tag2idx, dev_sentences, dev_labels, tokenizer_path = bert_tokenizer, do_lower_case=do_lower_case)
...
def generate_test_data(config, tag2idx, bert_tokenizer="bert-base", do_lower_case=True):
    ...
    test_dataset = disf_Dataset(tag2idx, test_sentences, test_labels, tokenizer_path = bert_tokenizer, do_lower_case=do_lower_case)
